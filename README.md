# OTT 콘텐츠 통합 검색 서비스
![couchpotato](https://github.com/user-attachments/assets/b6d4d6ff-b648-429b-838a-b20b8ca0080c)

<br>

## 프로젝트 소개
- 여러 OTT 플랫폼의 콘텐츠 정보를 통합해 사용자들이 한 곳에서 편하게 검색하고 비교할 수 있는 서비스입니다.
- OTT 플랫폼별 콘텐츠 랭킹(TOP 10)의 정보를 제공합니다.
- 콘텐츠 정보를 검색하고, 그에 대한 리뷰를 등록할 수 있습니다.
- 콘텐츠별로 나의 리뷰, 팔로우의 리뷰, 전체 리뷰를 볼 수 있습니다. 
- 리뷰 작성 시 캘린더를 통해서 날짜별로 리뷰를 관리하고, 월별로 나의 리뷰를 모아볼 수 있습니다.
- 콘텐츠 N회차 리뷰 작성 시, 회차 정보를 제공합니다.
- 다양한 유저들을 팔로우하고, 친구들이 감상한 콘텐츠 및 리뷰를 볼 수 있습니다.

<br>

## 팀원 구성
<div>

| **선푸름(팀장)** | **김호영** | **박유은** | **조성현** |
| :------: |  :------: | :------: | :------: |
| <img src="https://github.com/user-attachments/assets/ae98ce5b-74bc-4923-8667-ede3a24c56b5?v=4" height="150" width="150"> | <img src="https://github.com/user-attachments/assets/27571ece-ea40-4b85-a595-0069736b684d?v=4" height="150" width="150"> | <img src="https://github.com/user-attachments/assets/181f827d-5d71-4ea5-9d54-12be70a23f2e?v=4" height="150" width="150"> | <img src="https://github.com/user-attachments/assets/5f5d1bf6-0a1b-4162-82ae-0292b991b1e0?v=4" height="150" width="150"> |

</div>

<br>

## 개발 기간
- 2024-07-02 ~ 2024-07-19

<br>

## 개발 환경
<span><strong>프론트엔드 (Frontend)</strong></span>
- <span>
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white"/>
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black"/>
  <img src="https://img.shields.io/badge/AJAX-007EC6?style=for-the-badge&logo=xml&logoColor=white"/>
</span>

<span><strong>백엔드 (Backend)</strong></span>
- <span>
  <img src="https://img.shields.io/badge/Spring-6DB33F?style=for-the-badge&logo=spring&logoColor=white"/>  
  <img src="https://img.shields.io/badge/JSP-007396?style=for-the-badge&logo=java&logoColor=white"/>
  <img src="https://img.shields.io/badge/MyBatis-BF0024?style=for-the-badge&logo=mybatis&logoColor=white"/>
  <img src="https://img.shields.io/badge/JDBC-5382A1?style=for-the-badge&logo=java&logoColor=white"/>
</span>

<span><strong>API</strong></span>
- <span>
   <img src="https://img.shields.io/badge/TMDB-01d277?style=for-the-badge&logo=tmdb&logoColor=white"/>
</span>

<span><strong>개발 도구 (IDE)</strong></span>
- <span>
  <img src="https://img.shields.io/badge/Eclipse-2C2255?style=for-the-badge&logo=eclipse-ide&logoColor=white"/>
  <img src="https://img.shields.io/badge/Maven-C71A36?style=for-the-badge&logo=apache-maven&logoColor=white"/>
</span>

<span><strong>서버</strong></span>
- <span>
  <img src="https://img.shields.io/badge/Tomcat-F8DC75?style=for-the-badge&logo=apache-tomcat&logoColor=black"/>
</span>

<span><strong>데이터베이스</strong></span>
- <span>
  <img src="https://img.shields.io/badge/Oracle-F80000?style=for-the-badge&logo=oracle&logoColor=white"/>
</span>

<span><strong>버전 관리 및 협업 도구</strong></span>
- <span>
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  <img src="https://img.shields.io/badge/Notion-000000?style=for-the-badge&logo=notion&logoColor=white"/>
</span>

<br>

## 역할 분담
### 🥔 선푸름

- **기능**
  - 리뷰 작성 시 캘린더와 연동(리뷰 캘린더), 날짜별 리뷰 보기, 월별 리뷰 모아보기
  - 리뷰 수정 및 삭제, 콘텐츠 N회차 리뷰 작성 시에 회차 정보를 제공
  - 팔로우의 리뷰 캘린더 및 리뷰 보기
  - 전체 CSS
<br>

### 🥔 김호영

- **기능**
  - 로그인, 회원가입, 마이 페이지, 비밀변호 변경, 로그아웃
  - 프로필 설정 및 수정(이미지 업로드)
  - 유저 아이디 중복 검사
<br>

### 🥔 박유은

- **기능**
  - 팔로잉 & 언팔로우, 팔로워, 사용자 전체 검색
  - 팔로잉 및 팔로워 수를 카운트하여 버튼에 표시
  - 팔로우 모달창
<br>

### 🥔 조성현

- **기능**
  - TMDB API 사용해서 콘텐츠 정보 가지고 오기
  - 각 플랫폼별 랭킹(TOP 10), 컨텐츠 검색, 컨텐츠 상세 보기
  - 리뷰 작성, 내 리뷰보기, 콘텐츠별 리뷰 보기, 팔로우의 리뷰 보기
<br>

##

